          ...
          .vuecal__flex(row grow)
            ...
            .vuecal__flex.vuecal__cells(
              ...
              .vuecal__flex.vuecal__split-days-headers(v-else-if="hasSplits && stickySplitLabels && minSplitWidth"
                :style="cellOrSplitMinWidth ? `min-width: ${cellOrSplitMinWidth}px` : ''")
                .day-split-header(v-for="(split, i) in daySplits" :key="i" :class="split.class || false" :style="isDayView && headingsWidthDay && headingsWidthDay?.[split.id] && headingsWidthDay?.[split.id]?.width? headingsWidthDay?.[split.id]?.width  : ''")
                  slot(name="split-label" :split="split" :view="view.id" :sizes="isDayView && headingsWidthDay && headingsWidthDay?.[split.id] && headingsWidthDay?.[split.id]?.width? headingsWidthDay?.[split.id]?.width  : ''") {{ split.label }}
              all-day-bar(
